...
static void show_sat_val(FILE *fp)
{
	...
	for (j = 0; j < wsize; j++) {
	    ...
	    val += (1L<<j) * (j < wsize-1 || isunsigned ? 1 : -1) * bitval;
	}
	if (isunsigned)
	    fprintf(fp, "%s:0x%.8x\n", ipi_buf[i].name, (unsigned) val);
	else 
	    fprintf(fp, "%s:0x%.16lxL\n", ipi_buf[i].name, val);
    }
}
...
int gen_solve(FILE *fp, op_ptr arg)
{
	...
	case OP_IPI:
	    ...
	    index = Cudd_NodeReadIndex(result);
	    if (index > maxindex)
		maxindex = index;
#if 0
	    printf("%d PI %s%c\n",
	...
	if (Cudd_bddPickOneCube(bdd_manager, result, buf)) {
	    int i;
	    for (i = 0; i < nvars; i++) {
		unsigned index = Cudd_NodeReadIndex(eval_buf[i].bdd_val);
		if (buf[index] == 2)
		    buf[index] = 0;
...
void dump_blif(FILE *fp, int funct_cnt, op_ptr *funct_set, char **funct_names)
{
  ...
  if (!Cudd_DumpBlif(bdd_manager, funct_cnt, funct_bdds, var_names, funct_names, NULL, fp, 0)) {
